...
export default function BuslineEditor({
	open,
	name,
	handleClose,
	setNameAndId,
	...
	function saveBusline() {
		if (!displayedName || displayedName == "") {
			...
			!name &&
			allBusstops.filter((stop) => stop.nrReihenfolge).length < 2
		) {
			setIsError(true);
		...
		if (name) {
			apiService
				.updateBusline(displayedName, id)
				.then((res) => {
		...
		<>
			{busstops && (
				<Dialog open={open} onClose={handleClose}>
					<DialogTitle className="buslineEditorTitle">
						Buslinien-Editor
					</DialogTitle>
					<DialogContent className="editorContent">
						<DialogContentText>
							{name
								? `Buslinie ${name} editieren`
						...
						{name && (
							<>
								...
								<Button
									...
									onClick={() => setDeleteStopOpen(true)}
								>
			...
			<AddStopDialog
				open={deleteStopOpen}
				line={{ name: name, id: id }}
				handleClose={() => setDeleteStopOpen(false)}
				isAdmin={isAdmin}
				isDelete={true}
			></AddStopDialog>
		</>
