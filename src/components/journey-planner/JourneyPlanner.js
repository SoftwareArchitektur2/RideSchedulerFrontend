	...
	function onSearchJourney() {
		if (
			...
			apiService
				.getJourneyPlan(
					selectedStop.id,
					startingTime.toISOString(true),
					moment
						.duration(scheduleTime, "HH:mm")
						.as("minutes")
						.toString()
				)
				.then((res) => {
		...
		<>
			...
			<Box>
				...
				>
					...
					>
						<FormControl>
							<InputLabel id="journeyStopLabel">
								Starthaltestelle
							</InputLabel>
							<Select
								value={selectedStop}
						...
						<TimePicker
							...
							renderInput={(params) => (
								<TextField sx={{ width: "12vw" }} {...params} />
							)}
						/>
						<FormControl>
							<InputLabel id="scheduleTimeLabel">
								Zeithorizont in Stunden
							</InputLabel>
							<Select
								value={scheduleTime}
